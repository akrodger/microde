...
void mcrd_eval_bgsp(mcrd_vec *x_old, mcrd_vec *x_new, mcrd_vec *x_cmp,
                    mcrd_vec *vFld_old, mcrd_vec *vFld_new, mcrd_vec *vFld_stg1,
                    mcrd_vec *vFld_stg2, mcrd_flt dt,
  ...
  mcrd_lincombo(x_new, ptrWrk, scalWrk, 4, 1.0, x_old, dt * bgsp_b_1, vFld_old,
                dt * bgsp_b_2, vFld_stg1, dt * bgsp_b_3, vFld_stg2);
  vecField(x_new, vFld_new, 0);
  mcrd_lincombo(x_cmp, ptrWrk, scalWrk, 5, 1.0, x_old, dt * bgsp_e_1, vFld_old,
                dt * bgsp_e_2, vFld_stg1, dt * bgsp_e_3, vFld_stg2,
                dt * bgsp_e_4, vFld_new);
}

// helper function for evaluating the Dormand * Prince 5(4) method.
void mcrd_eval_dopr(mcrd_vec *x_old, mcrd_vec *x_new, mcrd_vec *x_cmp,
                    mcrd_vec *vFld_old, mcrd_vec *vFld_new, mcrd_vec *vFld_stg1,
                    mcrd_vec *vFld_stg2, mcrd_vec *vFld_stg3,
                    mcrd_vec *vFld_stg4, mcrd_flt dt,
                    void (*vecField)(mcrd_vec *, mcrd_vec *, int, ...)) {
  // Butcher tableau constants for Bogacki-Shampine method
  // Butcher matrix:
  ...
  mcrd_lincombo(x_stg, ptrWrk, scalWrk, 3, 1.0, x_old, dt * dopr_a_31, vFld_old,
                dt * dopr_a_32, vFld_stg_extra);
  vecField(x_stg, vFld_stg1, 0);
  mcrd_lincombo(x_stg, ptrWrk, scalWrk, 4, 1.0, x_old, dt * dopr_a_41, vFld_old,
                dt * dopr_a_42, vFld_stg_extra, dt * dopr_a_43, vFld_stg1);
  vecField(x_stg, vFld_stg2, 0);
  mcrd_lincombo(x_stg, ptrWrk, scalWrk, 5, 1.0, x_old, dt * dopr_a_51, vFld_old,
                dt * dopr_a_52, vFld_stg_extra, dt * dopr_a_53, vFld_stg1,
                dt * dopr_a_54, vFld_stg2);
  vecField(x_stg, vFld_stg3, 0);
  mcrd_lincombo(x_stg, ptrWrk, scalWrk, 6, 1.0, x_old, dt * dopr_a_61, vFld_old,
                dt * dopr_a_62, vFld_stg_extra, dt * dopr_a_63, vFld_stg1,
                dt * dopr_a_64, vFld_stg2, dt * dopr_a_65, vFld_stg3);
  vecField(x_stg, vFld_stg4, 0);
  mcrd_lincombo(x_new, ptrWrk, scalWrk, 6, 1.0, x_old, dt * dopr_b_1, vFld_old,
                dt * dopr_b_3, vFld_stg1, dt * dopr_b_4, vFld_stg2,
                dt * dopr_b_5, vFld_stg3, dt * dopr_b_6, vFld_stg4);
  vecField(x_new, vFld_new, 0);
  mcrd_lincombo(x_cmp, ptrWrk, scalWrk, 7, 1.0, x_old, dt * dopr_e_7, vFld_new,
                dt * dopr_e_1, vFld_old, dt * dopr_e_3, vFld_stg1,
                dt * dopr_e_4, vFld_stg2, dt * dopr_e_5, vFld_stg3,
                dt * dopr_e_6, vFld_stg4);
}

// compute dense outpute polynomial spline for the DOPRI(5,4) method.
void mcrd_dopr_spline(mcrd_vec *spline, mcrd_vec *x_old, mcrd_vec *vFld_old,
                      mcrd_vec *vFld_new, mcrd_vec *vFld_stg1,
                      mcrd_vec *vFld_stg2, mcrd_vec *vFld_stg3,
                      mcrd_vec *vFld_stg4, mcrd_flt dt, mcrd_flt theta) {
  static const mcrd_flt dopr_b_1 = 35.0 / 384.0;
  static const mcrd_flt dopr_b_3 = 500.0 / 1113.0;
  static const mcrd_flt dopr_b_4 = 125.0 / 192.0;
  static const mcrd_flt dopr_b_5 = -2187.0 / 6784.0;
  ...
  mcrd_flt b1 = (theta * theta) * (3.0 - (2.0 * theta)) * dopr_b_1 +
                ...
                (theta * theta) * ((theta - 1) * (theta - 1)) * 5.0 *
                    (2558722523 - (31403016 * theta)) / 11282082423;
  ...
  mcrd_flt b7 = (theta * theta) * (theta - 1) +
                (theta * theta) * ((theta - 1) * (theta - 1)) * 10.0 *
                    (7414447.0 - (829305 * theta)) / 29380423.0;
  mcrd_lincombo(spline, ptrWrk, scalWrk, 7, 1.0, x_old, dt * b1, vFld_old,
                dt * b7, vFld_new, dt * b3, vFld_stg1, dt * b4, vFld_stg2,
                dt * b5, vFld_stg3, dt * b6, vFld_stg4);
}
...
mcrd_flt mcrd_step_select(mcrd_flt dt, mcrd_flt err_old, mcrd_flt err_new,
                          mcrd_flt tol, mcrd_int p) {
  mcrd_flt step_select;
  if (err_new > 0.0) {
    step_select =
        pow(tol / err_new, 1.0 / (p + 1.0)) * pow(err_old / tol, 0.08);
...
void mcrd_copy(mcrd_vec *dest, mcrd_vec *src) {
  ...
  if (src->n == dest->n) {
#if defined(SHMEM_PARA_MICRODE)
#pragma omp parallel default(shared) private(i, chunkSize, N, loopStart, \
                                             loopEnd, chunkRem, thNum, thTot)
    {
...
#pragma omp barrier
    ...
    fprintf(stderr,
            "%s:Line %d %s::mcrd_copy::"
            "dest->n = %ld, src->n = %ld",
            __FILE__, __LINE__, __func__, (long)dest->n, (long)src->n);
    exit(1);
  }
}
...
void mcrd_axpby(mcrd_flt a, mcrd_vec *x, mcrd_flt b, mcrd_vec *y, mcrd_vec *z) {
  ...
  if (x->n == y->n && y->n == z->n) {
    ...
    fprintf(stderr,
            "%s:Line %d %s::mcrd_axpby::"
            "x->n = %ld, y->n = %ld, z->n = %ld",
            __FILE__, __LINE__, __func__, (long)x->n, (long)y->n, (long)z->n);
    exit(1);
  }
}
...
void mcrd_lincombo(mcrd_vec *x, mcrd_flt **ptrWrk, mcrd_flt *scalWrk,
  ...
  for (j = 0; j < numTerms; j++) {
    ...
    if (x->n != z->n) {
      fprintf(stderr,
              "%s:Line %d %s::mcrd_lincombo::"
              "x->n = %ld, term = %ld, z->n = %ld",
              __FILE__, __LINE__, __func__, (long)x->n, (long)j, (long)z->n);
      exit(1);
    }
  }
#if defined(SHMEM_PARA_MICRODE)
#pragma omp parallel default(shared) private(i, j, a, e, y, t, N, loopStart, \
                                             loopEnd, chunkRem, thNum, thTot)
  {
    thTot = omp_get_num_threads();
    thNum = omp_get_thread_num();
...
mcrd_flt mcrd_mse(mcrd_vec *x, mcrd_vec *y) {
  ...
  if (x->n != y->n) {
    fprintf(stderr,
            "%s:Line %d %s::mcrd_mse::"
            "x->n = %ld, y->n = %ld",
            __FILE__, __LINE__, __func__, (long)x->n, (long)y->n);
    exit(1);
  }
#if defined(SHMEM_PARA_MICRODE)
...
void mcrd_ode_solve_o1(mcrd_vec *x_init, mcrd_vec *x_snap, mcrd_flt *t,
  ...
  k = 1;
  while (breakflag == 0) {
    mcrd_o1_autostep(&x_old, &x_new, &vFld_old, &vFld_new, &x_cmp, &dt1, &dt2,
                     vecField, &err_old, &err_new, &n1, &n2, abs_tol, rel_tol);
    n1 = n2;
    time_new = time_now + dt1;
    if (time_new > t[t_len - 1] && k == t_len - 1) {
      mcrd_axpby(1.0, &x_old, t[t_len - 1] - time_now, &vFld_old,
                 &(x_snap[t_len - 1]));
      break;
    }
...
void mcrd_o2_autostep(mcrd_vec *x_old, mcrd_vec *x_new, mcrd_vec *vFld_old,
  ...
  err_old_ptr[0] = err_old;
  do {
    mcrd_eval_bgsp(x_old, x_new, x_cmp, vFld_old, vFld_new, vFld_stg1,
                   vFld_stg2, dt, vecField);
    err_old = err_new;
    err_new = mcrd_mse(x_new, x_cmp);
    nrm_new = mcrd_rms(x_new);
    *dt_old_ptr = dt;
...
void mcrd_ode_solve_o2(mcrd_vec *x_init, mcrd_vec *x_snap, mcrd_flt *t,
                       ...
                       mcrd_flt abs_tol, mcrd_flt rel_tol, mcrd_flt *workVec) {
  static const mcrd_flt eps1 = 1e-5, eps2 = 1e-6, eps3 = 1e-15, eps4 = 1e-3;
  static mcrd_vec x_old, x_new, vFld_old, vFld_new, x_cmp, vFld_stg1, vFld_stg2;
  ...
  k = 1;
  while (breakflag == 0) {
    mcrd_o2_autostep(&x_old, &x_new, &vFld_old, &vFld_new, &vFld_stg1,
                     &vFld_stg2, &x_cmp, &dt1, &dt2, vecField, &err_old,
                     &err_new, &n1, &n2, abs_tol, rel_tol);
    n1 = n2;
    time_new = time_now + dt1;
    // use polynomial interponation for dense output.
    while (time_now <= t[k] && t[k] < time_new) {
      ...
      mcrd_lincombo(&(x_snap[k]), ptrWrk, scalWrk, 4, m1, &x_old, m2, &x_new,
                    m3, &vFld_old, m4, &vFld_new);
      k++;
      if (k >= t_len) {
        breakflag = 1;
        break;
...
void mcrd_o4_autostep(mcrd_vec *x_old, mcrd_vec *x_new, mcrd_vec *vFld_old,
  ...
  err_old_ptr[0] = err_old;
  do {
    mcrd_eval_dopr(x_old, x_new, x_cmp, vFld_old, vFld_new, vFld_stg1,
                   vFld_stg2, vFld_stg3, vFld_stg4, dt, vecField);
    err_old = err_new;
    err_new = mcrd_mse(x_new, x_cmp);
    nrm_new = mcrd_rms(x_new);
    *dt_old_ptr = dt;
...
void mcrd_ode_solve_o4(mcrd_vec *x_init, mcrd_vec *x_snap, mcrd_flt *t,
  ...
  k = 1;
  while (breakflag == 0) {
    mcrd_o4_autostep(&x_old, &x_new, &vFld_old, &vFld_new, &vFld_stg1,
                     &vFld_stg2, &vFld_stg3, &vFld_stg4, &x_cmp, &dt1, &dt2,
                     vecField, &err_old, &err_new, &n1, &n2, abs_tol, rel_tol);
    n1 = n2;
    time_new = time_now + dt1;
    // use polynomial interponation for dense output.
    while (time_now <= t[k] && t[k] < time_new) {
      theta = (t[k] - time_now) / dt1;
      mcrd_dopr_spline(&(x_snap[k]), &x_old, &vFld_old, &vFld_new, &vFld_stg1,
                       &vFld_stg2, &vFld_stg3, &vFld_stg4, dt1, theta);
      k++;
      if (k >= t_len) {
        breakflag = 1;
        break;
